...
function draw(){
  ...
  for(let t=startT;t<=endT;t++){
    ...
    for(const [key,pid] of Object.entries(LOOKUP)){
      ...
      const oid=ownership[pid];
      const isSea=byId[pid]?.terrain==='sea';
      let fill=null;
      if(selProv&&pid===selProv.id){
        fill= isSea ? 'rgba(90,160,255,.5)' : 'rgba(0,212,255,.4)';
        ctx.strokeStyle= isSea ? 'rgba(90,160,255,.95)' : 'rgba(0,212,255,.95)';
        ctx.lineWidth=1;
        ctx.strokeRect(x+.5,y+.5,cellW-1,cellH-1);
      } else if(isSea){
        // Sea province — always show subtle blue tint
        fill='rgba(40,100,200,.18)';
...
function renderNavalTab(){
  ...
  const seaList='<div style="font-size:7px;color:rgba(90,180,255,.3);letter-spacing:.1em;margin:8px 0 3px">SEA PROVINCES ('+seas.length+')</div>'
    ...
    +seas.map(p=>{
      return '<div onclick="openProvModal('+p.id+')" style="font-size:7.5px;color:rgba(90,180,255,.5);padding:3px 6px;border:1px solid rgba(90,180,255,.12);cursor:pointer;background:rgba(40,100,200,.06)" title="Click for details">'
        +'\u{1F30A} '+p.name.split(' ').slice(-1)[0]+'<br>'
        +'<span style="font-size:6.5px;color:rgba(90,180,255,.3)">\u{1F4B0}+'+p.gold+' \u2699+'+p.supply+'</span>'
        +'</div>';
    }).join('')
    +'</div>';
